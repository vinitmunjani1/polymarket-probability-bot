...
class SpotModel:
    ...
    async def features(self, asset: str, start: int, seconds_left: float, price_to_beat: float | None = None) -> SpotFeatures:
        ...
        # Polymarket resolves against Chainlink/Vatic price-to-beat, not the
        # Binance candle open. Binance remains useful for current fast spot and
        # volatility/momentum, but the decision boundary must be the oracle target.
        open_px = float(price_to_beat) if price_to_beat else (self._window_open(rows, start) or float(rows[-1][1]))
